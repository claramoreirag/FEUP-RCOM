...
int send_ua(int fd, char flag)
{
  	unsigned char buf[5];
    buf[0]=FLAG;
    if(flag == TRANSMITTER) buf[1] = A_RSP_EMISSOR;
    else if (flag == RECEIVER) buf[1] = A_RSP_RECETOR;
    else return -1;
    buf[2]=UA;
...
int createInfoFrame(char * buf, int s, char * frame){
  ...
  char bcc = calculateBCC2(buf);
  frame[4+strlen(res)]=bcc;
  frame[5+strlen(res)]=FLAG;
